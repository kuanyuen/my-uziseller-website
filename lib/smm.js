...
export function publicServices(data) {
  const vndToMyr = Number(process.env.SMM_VND_TO_MYR_RATE || process.env.VND_TO_MYR_RATE || 0.000156);
  const usdToMyr = Number(process.env.SMM_USD_TO_MYR_RATE || process.env.USD_TO_MYR_RATE || 4.04);
  const cnyToMyr = Number(process.env.SMM_CNY_TO_MYR_RATE || process.env.CNY_TO_MYR_RATE || 0.60);
  const MARKUP_PERCENT = Number(process.env.SMM_MARKUP_PERCENT || 30);
  ...
  return normalizeServices(data)
    ...
    .map(s => {
      ...
      return {
        ...
        basePriceMYR: Math.round(myr * 100) / 100,
...
export async function getSupplierOrderStatus(orderId) {
  const { data } = await callSmm({ action: 'status', order: orderId });
  return data;
}

export async function placeOrder({ service, link, quantity, comments = '', schedule = '', dripfeed = false, runs = 2, interval = 10 }) {
  const form = { action: 'add', service, link, quantity };
  if (comments) form.comments = comments;
  if (schedule) form.schedule = schedule;
  if (dripfeed) {
    form.dripfeed = 1;
    form.runs = Math.max(2, Number(runs) || 2);
    form.interval = Math.max(1, Number(interval) || 10);
  }
  const { data } = await callSmm(form);
  return data;
}
